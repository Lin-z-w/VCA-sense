...
def extract_rtp_on_port(pcap_file, tshark_path, port, seen_frames, verbose=False):
    ...
    for line in out.splitlines():
        if not line.strip():
            continue
        parts = line.split("|")
        # 预期 7 个字段: frame | time | ip.src | udp.src | ip.dst | udp.dst | rtp.seq
        if len(parts) < 7:
            continue
        try:
...
def save_grouped_csv(records, outdir):
    ...
    for key, recs in grouped.items():
        ...
        with open(outfile, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["frame", "time", "ip_src", "udp_src", "ip_dst", "udp_dst", "rtp_seq", "port_forced"])
            for r in recs:
                w.writerow([r["frame"], r["time"], r["ip_src"], r["udp_src"], r["ip_dst"], r["udp_dst"], r["rtp_seq"], r["port_forced"]])
